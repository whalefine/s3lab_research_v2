...
// Shadow SRAM (behavioral mirror of CLK(clk) macros' contents)
...
// Shadow SRAM mirrors the CLK(clk) macros' memory contents (posedge write only).
// Comparisons read SHADOW_* by index at stage edges, so no read-address latch is
// needed (the macros' Q latency does not affect the shadow's stored values).
