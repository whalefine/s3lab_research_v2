...
// inv_sqrt_nr.v
//
// Q8.8 inverse square root via Newton-Raphson（2 次迭代）。
//
...
//   - FSM：IDLE → LOAD → ITER1 → ITER2 → DONE
//   - 乘法使用 32-bit 中間暫存器（v 與 y 皆為 Q8.8，積為 Q16.16，>>8 = Q8.8）
//
...
// y² in Q8.8: (y × y) >> 8 — keep Q16.16 intermediate, take [23:8]
wire [31:0] y_sq_raw   = $signed(y_reg) * $signed(y_reg);
wire signed [15:0] y_sq = y_sq_raw[23:8];

// 0.5 × v × y² in Q8.8: (v × y_sq) >> 9
wire [31:0] term_raw   = $signed(v_reg) * $signed(y_sq);
wire signed [15:0] term = term_raw[24:9];

// 1.5 - 0.5×v×y² in Q8.8 (1.5 = 384)
wire signed [15:0] coeff = 16'sd384 - term;

// y_new = y × coeff in Q8.8
...
always @(posedge clk) begin
    ...
    if (reset) begin
        ...
        case (state)
            ...
            S_ITER1: begin
                // Apply first NR iteration: y = y × (1.5 - 0.5×v×y²)
                y_reg <= y_new;
            end
            S_ITER2: begin
                // Apply second NR iteration
                y_reg <= y_new;
            end
            S_DONE: begin
                y_o  <= y_reg;
