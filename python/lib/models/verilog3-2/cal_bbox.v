...
always @(posedge clk) begin
    if (!rst_n) begin
        ...
        max_idx_r <= 8'd0 ;
        ctr_cnt   <= 9'd0 ;
    end else if (ctr_in_valid && (ctr_in_data > max_val_r)) begin
        max_val_r <= ctr_in_data ;
        max_idx_r <= ctr_cnt[7:0] ;
        ctr_cnt   <= ctr_cnt + 9'd1 ;
    end else if (ctr_in_valid)
        ctr_cnt <= ctr_cnt + 9'd1 ;
end
...
// Sram_qkm mux: drive A/CEB/WEB/D *combinationally* (no _n->reg stage) for the
// CLK(clk) registered-data macro. The 4 emit reads issue addr one cycle earlier than
// the old CLK(~clk)+registered-addr path, so so_q latency stays 1 cycle and the
// B_RD_* latch timing (off_x_r<=so_q etc.) is unchanged. Streaming writes (size/off)
// land at posedge instead of negedge (~half cycle earlier), all before start.
always @(*) begin
    so_ceb  = 1'b1 ;
    so_web  = 1'b1 ;
    so_addr = {QKM_AW{1'b0}} ;
    ...
    if (size_in_valid) begin
        so_ceb  = 1'b0 ;
        so_web  = 1'b0 ;
        so_addr = {{(QKM_AW-SO_AW){1'b0}}, size_wr_addr} ;
        so_din  = size_in_data ;
    end else if (off_in_valid) begin
        so_ceb  = 1'b0 ;
        so_web  = 1'b0 ;
        so_addr = {{(QKM_AW-SO_AW){1'b0}}, off_wr_addr} ;
        so_din  = off_in_data ;
    end else begin
        case (CS)
            B_IDLE : begin
                if (start) begin
                    so_ceb  = 1'b0 ;
                    so_web  = 1'b1 ;
                    so_addr = {{(QKM_AW-SO_AW){1'b0}}, rd_off_x_addr} ;
                end
            end
            B_RD_OFF_X : begin
                so_ceb  = 1'b0 ;
                so_web  = 1'b1 ;
                so_addr = {{(QKM_AW-SO_AW){1'b0}}, rd_off_y_addr} ;
            end
            B_RD_OFF_Y : begin
                so_ceb  = 1'b0 ;
                so_web  = 1'b1 ;
                so_addr = {{(QKM_AW-SO_AW){1'b0}}, rd_size_w_addr} ;
            end
            B_RD_SZ_W : begin
                so_ceb  = 1'b0 ;
                so_web  = 1'b1 ;
                so_addr = {{(QKM_AW-SO_AW){1'b0}}, rd_size_h_addr} ;
            end
            default : ;
        endcase
    end
