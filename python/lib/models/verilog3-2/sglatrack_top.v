// =============================================================================
// sglatrack_top.v -- verilog3 merged top (backbone3 + head3, 6 shared SRAMs)
//
// Plan B: backbone norm stays in Sram_tok1; head conv1 reads Sram_tok1 directly.
// Global MUX (phase_sel): 0=backbone, 1=head. TS_GUARD registers 1 cycle before head.
// No tok1_preload / tok1_readback (full-chain: template+search stream -> backbone -> head).
//
// SRAM macro sharing (6 macros):
//   Sram_tok1    | backbone inter-block + bb norm     | head conv1 activation read
//   Sram_tok2    | transformer x_buf                  | head sh2
//   Sram_q       | care attn q + tmp                  | head sh1_lo
//   Sram_16384 k | care k + MLP scratch               | head sh1_hi
//   Sram_16384 v | care v + MLP scratch               | head x_buf
//   Sram_16384 qkm | care qkm + MLP scratch           | head cal_bbox maps
//
// Top FSM: TS_IDLE -> TS_BACKBONE -> TS_GUARD -> TS_HEAD -> TS_DONE
//
// VCS (compile root with memory/ ROM + SRAM macros):
//   vcs python/lib/models/verilog3/sglatrack_top.v python/lib/models/verilog3/TEST.v \
//       memory/Sram_tok1.v memory/Sram_tok2.v memory/Sram_q.v memory/Sram_16384.v \
//       memory/rom_*.v +lint=TFIPC-L +define+TSMC_CM_NO_WARNING
// =============================================================================
...
// Purpose: top FSM state register
...
// Purpose: phase_sel and sub-module start pulses
always @(posedge clk) begin
    bb_start_r <= 1'b0;
    hd_start_r <= 1'b0;
...
// =========================================================================
// Shared SRAM macro ports (after phase_sel MUX)
// =========================================================================
...
// phase_sel 0=backbone, 1=head (mutually exclusive SRAM masters)
...
// =========================================================================
// head3 instance (reads norm from Sram_tok1; no external activation port)
// =========================================================================
head_top #(
    .IN_CH    (EMBED_DIM),
    .C_SH1    (C_SH1),
...
// =========================================================================
// 6 shared SRAM macros (CLK = clk; posedge registered-data, addr@T -> Q@T+1).
// backbone_top / head_top drive A/CEB/WEB/D combinationally, so the read latency
// stays 1 cycle (issue@T -> Q@T+1), matching the standalone backbone3-2/head3-2 tops.
// =========================================================================
