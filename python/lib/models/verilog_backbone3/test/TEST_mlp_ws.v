...
// k/v/qkm/ao SRAM:  same 1-cycle read latency as care_attention TB.
//
// Quick smoke (1 token):
...
// ROM / norm2: addr @ posedge T -> data @ posedge T+1
// WLOAD must latch local addr 0 (neu0 feat0); do not gate on !=0 (TEST_mlp_vec8 style).
always @(negedge clk) begin
    if (norm_rd_en)
        norm_addr_rom_q <= norm_rd_flat;
    if (u_dut.state == 4'd1)
        w_addr_rom_q <= wgt_addr_o[12:0];
    // BLOAD phase0: latch bias addr including 0 (FC2 group0 bl0 after FC1 leaves stale b_addr=127)
    if (u_dut.state == 4'd2 && u_dut.bl_phase == 1'b0)
        b_addr_rom_q <= bias_addr_o;
end

// SRAM read: addr @ posedge T -> Q @ posedge T+1
always @(negedge clk) begin
