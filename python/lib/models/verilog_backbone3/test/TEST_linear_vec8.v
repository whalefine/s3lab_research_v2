...
//
// ROM model: addr @ posedge T -> w_i/b_i valid @ T+1 (CLK=~clk contract).
//
...
// ROM model: addr driven @ posedge T, data valid @ posedge T+1 (CLK=~clk contract).
// Latch addr on negedge (after NBA) so capture at T+1 sees the issued address.
...
assign w_i = $signed(W_MEM[w_addr_rom_q]);
assign b_i = $signed(B_MEM[b_addr_rom_q]);

always #(CYCLE/2.0) clk = ~clk;

always @(negedge clk) begin
    w_addr_rom_q <= w_addr_o;
