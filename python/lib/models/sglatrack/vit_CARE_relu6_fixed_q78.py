...
本檔為 vit_CARE_relu6_fixed 之 Q7.8 變體（檔名 ``vit_CARE_relu6_fixed_q78.py``）：全路徑 ``to_fixed_point(..., 7, 8)``。不修改 ``vit_CARE_relu6_fixed.py``（仍為 Q8.8）。
"""
import math
import logging
...
def vit_base_patch16_224(pretrained=False, **kwargs):
    """
    ViT-Base (ViT-B/16)，與 vit.py 相同結構與權重鍵名；Attention 為 CARE ReLU6 softmax-free 前向，並含 Q7.8（7,8）截斷。
    """
    model_kwargs = dict(
