...
本檔為 vit_CARE_relu6 dim256 之 Q8.8 變體（檔名 ``vit_CARE_relu6_dim256_fixed_q88.py``）：
embed_dim=256 / num_heads=8；全路徑 ``to_fixed_point(..., 8, 8)``。
與 ``vit_CARE_relu6_fixed_q88.py``（768 維）同結構，factory 與預訓練載入對齊 dim256 student。
"""
import math
import logging
...
from lib.models.sglatrack.vit_CARE_relu6_dim256 import (
    ...
    STUDENT_EMBED_DIM,
    STUDENT_NUM_HEADS,
    load_projected_pretrained_dim256,
)
...
def _create_vision_transformer_dim256_fixed_q88(pretrained=False, **kwargs):
    ...
    if pretrained:
        if isinstance(pretrained, str) and pretrained.endswith(".pth"):
            ...
            raise ValueError(
                "dim256 fixed Q8.8：pretrained 請為 MAE/ViT .pth 或 sglatrack checkpoint 路徑"
            )

    return model


def vit_care_dim256_fixed_q88_patch16_224(pretrained=False, **kwargs):
    """
    CARE ReLU6 Q8.8，embed_dim=256、depth=12、num_heads=8。
    state_dict 與浮點 ``vit_CARE_relu6_dim256`` 相容，供 test 時插入 Q8.8 截斷。
    """
    return _create_vision_transformer_dim256_fixed_q88(pretrained=pretrained, **kwargs)
