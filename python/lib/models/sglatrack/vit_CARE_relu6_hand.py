...
本檔由 vit_CARE_relu6.py 複製並改寫成 hand 版本，維持原 class/function 名稱，
並將主要 PyTorch/timm 元件替換為 python/lib/module 對應實作，方便硬體化檢視資料流。
"""
import math
import logging
...
from lib.module import Dropout, DropPath, LayerNorm, Linear, Mlp, relu6
from lib.models.layers.patch_embed import PatchEmbed
from lib.models.sglatrack.base_backbone_hand import BaseBackbone
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    if isinstance(model.head, (nn.Linear, Linear)) and model.head.bias.shape[0] == w[f'{prefix}head/bias'].shape[-1]:
        model.head.weight.copy_(_n2p(w[f'{prefix}head/kernel']))
        model.head.bias.copy_(_n2p(w[f'{prefix}head/bias']))
    if isinstance(getattr(model.pre_logits, 'fc', None), (nn.Linear, Linear)) and f'{prefix}pre_logits/bias' in w:
