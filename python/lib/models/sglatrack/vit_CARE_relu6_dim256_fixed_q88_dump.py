"""Dump-only variant：`embed_dim=256`、`num_heads=8`、全路徑 Q8.8（``to_fixed_point(..., 8, 8)``）。

結構／state_dict 鍵名對齊 ``vit_CARE_relu6_dim256``／``vit_CARE_relu6_dim256_fixed_q88``；
dump 節點與路徑對齊 ``vit_CARE_relu6_dim32_fixed_dump.py``（AttentionDump / BlockDump /
VisionTransformerDump + ``forward_test_from_post_embed``）。

不可用於正式 training；僅供搭配 dim256 訓練 checkpoint 做 golden dump／RTL 對拍。

用法：
    cfg.MODEL.BACKBONE.TYPE: 'vit_care_relu6_dim256_fixed_q88_dump_base_patch16_224'
...
def vit_care_dim256_fixed_q88_dump_patch16_224(pretrained=False, **kwargs):
    """Dump-only CARE ReLU6 Q8.8，embed_dim=256、depth=12、num_heads=8。

    ``state_dict`` 鍵名與 layout 對齊 ``vit_CARE_relu6_dim256``（可載入同維度 student ckpt，
    ...
    model = _create_vision_transformer(
        "vit_care_dim256_fixed_q88_dump_patch16_224", pretrained=pretrained, **model_kwargs
    )
    return model
