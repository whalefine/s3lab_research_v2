"""Dump-only variant：`embed_dim=192`、`num_heads=3`、全路徑 Q7.7（``to_fixed_point(..., 7, 7)``）。

結構對齊 ``vit_CARE_relu6_dim192_fixed_q77.py`` + ``vit_CARE_relu6_dim32_fixed_shared_trunk_dump.py`` 之 dump 節點；
搭配 **shared-trunk head dump**（`CENTER_DUMP_SHARED_TRUNK` + ``head_shared_trunk_dump.py``，Q7.7）。

不可用於正式 training；僅供 golden dump／RTL 對拍。

用法：
    cfg.MODEL.BACKBONE.TYPE: 'vit_care_relu6_dim192_fixed_q77_shared_trunk_dump_base_patch16_224'
    experiments: ``vit_coco_got10k_distill_mae_teacher_orr_afkd_s60000_bs32_dim192_augreg_shared_trunk_fixed_q77_dump.yaml``
...
def _save_npy(enabled: bool, dump_dir: str, filename: str, tensor: torch.Tensor) -> None:
    """Dump tensor as .npy if enabled. 所有呼叫點都保證 tensor 已經過 Q7.7 量化。"""
...
class AttentionDump(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        qk_mean = to_fixed_point(qk_mean, 7, 7)
        # Q7.7 中 1e-5 會量化成 0，可能導致 reciprocal 前除零並傳播 NaN。
        # 改為 clamp 到最小正 Q7.7（1/256）以對齊 numpy 路徑的防護策略。
        qk_mean_eps = torch.clamp(qk_mean, min=1.0 / 128.0)
        qk_mean_eps = to_fixed_point(qk_mean_eps, 7, 7)
        z = 1.0 / qk_mean_eps
...
class VisionTransformerDump(BaseBackboneFix):
    ...
    def forward_test(self, z, x):
        ...
        z_patch = to_fixed_point(z_patch, 7, 7)
        x_patch = to_fixed_point(x_patch, 7, 7)
        self._dump(z_patch, "template_after_patch_embed_out.npy")
        self._dump(x_patch, "search_after_patch_embed_out.npy")

        z_pos = to_fixed_point(self.pos_embed_z, 7, 7)
        x_pos = to_fixed_point(self.pos_embed_x, 7, 7)
        self._dump(z_pos, "template_pos_embed.npy")
        self._dump(x_pos, "search_pos_embed.npy")

        z = to_fixed_point(z_patch + z_pos, 7, 7)
        x = to_fixed_point(x_patch + x_pos, 7, 7)
    ...
    def forward_test_from_post_embed(self, template_post_embed: torch.Tensor,
        ...
        之後、且已做過 Q7.7 截斷的張量。Python 端同步 dump
    ...
    def _forward_from_post_embed(self, z, x, B):
        ...
        for i, blk in enumerate(self.blocks):
            if i < start_layer:
                ...
                if len(idx) > 0:
                    x[idx] = blk(x[idx])
                    x[idx] = to_fixed_point(x[idx], 7, 7)
                    break
        ...
        x_norm = self.norm(x)
        x_norm = to_fixed_point(x_norm, 7, 7)
...
def vit_tiny192_care_fixed_q77_shared_trunk_dump_patch16_224(pretrained=False, **kwargs):
    """Dump-only CARE ReLU6 Q7.7，embed_dim=192、depth=12、num_heads=3。"""
    model_kwargs = dict(
        patch_size=16,
        embed_dim=192,
        depth=12,
    ...
    model = _create_vision_transformer(
        "vit_tiny192_care_fixed_q77_shared_trunk_dump_patch16_224", pretrained=pretrained, **model_kwargs
    )
    return model
