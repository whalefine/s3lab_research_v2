...
class ThreeLayerMLP(nn.Module):
    ...
    def forward(self, x):
        # ThreeLayerMLP 屬 adaptive selector（純軟體），不進 RTL，故保持 float。
        x = self.fc1(x)
        x = self.relu(x)
        x = self.fc2(x)
        pro = self.sigmoid(x)
...
class BaseBackboneFix(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        b_sz, emb, _ = patch_pos_embed.shape
        p_h = self.img_size[0] // self.patch_size
        p_w = self.img_size[1] // self.patch_size
        patch_pos_embed = patch_pos_embed.view(b_sz, emb, p_h, p_w)
        ...
        print(
            f"Initializing MLP with input_dim={total_tokens} (template patches: {lens_z}, search patches: {lens_x})"
        )
        self.MLP = ThreeLayerMLP(input_dim=total_tokens, output_dim=12 - 1 - start_layer)

    def _forward_impl(self, z, x, is_test: bool):
        """精簡版 fixed-point 插入點：
        - patch_embed 輸出（Conv MAC 結尾）
        - patch + pos_embed 殘差相加後
        - 每個 transformer block 的輸出（block 內部已有 norm/attn/mlp 的量化）
        - self.norm 最終輸出（送入 head 前的介面）

        combine_tokens / pos_drop(eval) / recover_tokens 只是 reshape/identity，
        不是 MAC 結尾，故不插。adaptive selector（ThreeLayerMLP / cosine）
        屬純軟體路徑，不進 RTL，維持 float。
        """
        b_sz = x.shape[0]

        z_patch = to_fixed_point(self.patch_embed(z), 8, 8)
        x_patch = to_fixed_point(self.patch_embed(x), 8, 8)

        z = to_fixed_point(z_patch + self.pos_embed_z, 8, 8)
        x = to_fixed_point(x_patch + self.pos_embed_x, 8, 8)
        ...
        with torch.no_grad():
            cos_tensor = torch.ones(b_sz, 12 - 1 - start_layer, device=x.device)
            for i, blk in enumerate(self.blocks):
                if i > start_layer:
                    temp = blk(mid)
        ...
        out = to_fixed_point(self.norm(x), 8, 8)
        aux_dict = {"attn": None, "cos_tensor": cos_tensor.detach(), "pro": pro}
        return out, aux_dict

    def forward_(self, z, x):
        return self._forward_impl(z, x, is_test=False)
    ...
    def forward_test(self, z, x):
        return self._forward_impl(z, x, is_test=True)
