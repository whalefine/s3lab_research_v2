...
MALA 風格 softmax-free 線性注意力，使用 ReLU6 kernel mapping（對照原版 ELU(x)+1），
...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        # φ(x) = relu6(x)  (kernel mapping；ReLU6 已非負，無需 +1)
        q = F.relu6(q)  # (B, h, N, d)
        k = F.relu6(k)  # (B, h, N, d)
...
def vit_base_patch16_224(pretrained=False, **kwargs):
    """
    ViT-Base (ViT-B/16)，與 vit.py 相同結構與權重鍵名；Attention 為 MALA ReLU6 softmax-free 前向。
    """
    model_kwargs = dict(
