"""Dump-only variant：`embed_dim=32`、`num_heads=4`（與 ``vit_CARE_relu6_dim32_fixed_dump.py`` 完全相同）。

內容與 ``vit_CARE_relu6_dim32_fixed_dump.py`` 共用同一套 AttentionDump / BlockDump /
VisionTransformerDump；另註冊 **不同 BACKBONE.TYPE** 字串，供搭配 **shared-trunk head dump**
（`CENTER_DUMP_SHARED_TRUNK` + ``head_shared_trunk_dump.py``）之 yaml 使用。

不可用於正式 training；僅供 golden dump／RTL 對拍。

用法：
    cfg.MODEL.BACKBONE.TYPE: 'vit_care_relu6_dim32_fixed_shared_trunk_dump_base_patch16_224'
    experiments: ``vit_coco_uav123_care_relu6_dim32_fixed_shared_trunk_dump.yaml``
...
def vit_tiny32_care_fixed_shared_trunk_dump_patch16_224(pretrained=False, **kwargs):
    """Dump-only CARE ReLU6，embed_dim=32（與 ``vit_tiny32_care_fixed_dump_patch16_224`` 為同一模型）。"""
    model_kwargs = dict(
        patch_size=16,
        embed_dim=32,
        depth=12,
    ...
    model = _create_vision_transformer(
        "vit_tiny32_care_fixed_shared_trunk_dump_patch16_224", pretrained=pretrained, **model_kwargs
    )
    return model
