"""Dump-only variant：`embed_dim=32`、`num_heads=4`，對齊 ``vit_CARE_relu6_dim32`` 結構／鍵名。

與 ``vit_CARE_relu6_fixed_dump.py`` **同一套 dump 類別與路徑**（AttentionDump / BlockDump /
VisionTransformerDump + ``forward_test_from_post_embed``），僅透過不同 factory 建 32 維 backbone。

設計原則與固定點規則同 ``vit_CARE_relu6_fixed_dump.py``；不可用於正式 training，
僅供搭配 dim32 訓練 checkpoint 做 golden dump／RTL 對拍。

用法：
    cfg.MODEL.BACKBONE.TYPE: 'vit_care_relu6_dim32_fixed_dump_base_patch16_224'
    experiments: ``vit_coco_uav123_care_relu6_dim32_fixed_dump.yaml``
...
class AttentionDump(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        # Q8.8 中 1e-5 會量化成 0，可能導致 reciprocal 前除零並傳播 NaN。
        # 改為 clamp 到最小正 Q8.8（1/256）以對齊 numpy 路徑的防護策略。
        qk_mean_eps = torch.clamp(qk_mean, min=1.0 / 256.0)
...
def vit_tiny32_care_fixed_dump_patch16_224(pretrained=False, **kwargs):
    """Dump-only CARE ReLU6，embed_dim=32、depth=12、num_heads=4。

    ``state_dict`` 鍵名與 layout 對齊 ``vit_CARE_relu6_dim32``（可載入同維度 student ckpt，
    ``strict=False`` 時略過 teacher-only 等鍵）。
    """
    model_kwargs = dict(
        patch_size=16,
        embed_dim=32,
    ...
    model = _create_vision_transformer(
        "vit_tiny32_care_fixed_dump_patch16_224", pretrained=pretrained, **model_kwargs
    )
    return model
