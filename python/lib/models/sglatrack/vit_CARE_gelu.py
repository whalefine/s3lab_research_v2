...
線性注意力，但將論文／vit_CARE 的 **φ(x)=ELU(x)+1** 改為 **φ(x)=GELU(x)+1** 以利對照實驗）。
ViT/DeiT 的 MLP 常用 GELU；**+1** 與其他變體一致，維持核與分母為正、數值穩定。
"""
import math
import logging
...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        # 再送入 GELU+1 正值核（對照 vit_CARE 的 ELU+1）
        s = self.scale ** 0.5
        qs = q * s
        ks = k * s
        q = F.gelu(qs) + 1.0
...
def vit_base_patch16_224(pretrained=False, **kwargs):
    """
    ViT-Base (ViT-B/16)，與 vit.py 相同結構與權重鍵名；Attention 為 GELU+1 核之 softmax-free 線性注意力。
    """
    model_kwargs = dict(
