// =============================================================================
// transformer_block.v  (verilog_dim256_backbone)
// -----------------------------------------------------------------------------
...
module transformer_block #(
    ...
    // Golden-Weight mux hint for TB / backbone_top:
    //   bit[9]   = norm_layer (0=norm1, 1=norm2)
    //   bits[7:0]= feat index (0..255); bit[8] reserved 0
...
reg        loadx_rd_en;
reg [16:0] loadx_rd_addr;
reg        loadx_wr_en;
reg [16:0] loadx_wr_addr;

reg [8:0]  tok_cnt;
reg        ln_start;
reg        ln_start_r;
reg [16:0] ln_cap_flat;

reg        attn_start;
reg [16:0] cap_ptr;

reg        mlp_start;
reg [16:0] mlp_cap_ptr;
...
// =========================================================================
wire        ca_norm_rd_en;
wire [16:0] ca_norm_rd_flat;
wire [17:0] ca_wgt_addr;
wire [9:0]  ca_bias_addr;
...
wire res_rd_active = (state == S_RES1 || state == S_RES2) &&
                     (res_sub == 2'd0) && (res_rp < TOK_FLAT[16:0]);
...
// =========================================================================
assign wgt_addr_o =
    (state == S_NORM1) ? {7'b000_0000, 3'b000, ln_feat_addr} :
    (state == S_NORM2) ? {7'b000_0000, 3'b001, ln_feat_addr} :
    (state == S_MLP_FEED || state == S_MLP_WAIT) ? mlp_wgt_addr :
    (state == S_ATTN_FEED || state == S_ATTN_WAIT) ? {2'b00, ca_wgt_addr} :
    20'd0;

assign bias_addr_o =
    (state == S_NORM1 || state == S_NORM2) ? {ln_feat_addr} :
    (state == S_MLP_FEED || state == S_MLP_WAIT) ? mlp_bias_addr :
    (state == S_ATTN_FEED || state == S_ATTN_WAIT) ? ca_bias_addr :
    10'd0;

assign norm_feat_addr_o =
    (state == S_NORM1) ? {1'b0, 1'b0, ln_feat_addr[7:0]} :
    (state == S_NORM2) ? {1'b1, 1'b0, ln_feat_addr[7:0]} :
    10'd0;
...
assign sram_k_addr_o = in_attn ? ca_k_addr : in_mlp ? mlp_k_addr : 17'd0;
...
assign sram_v_addr_o = in_attn ? ca_v_addr : in_mlp ? mlp_v_addr : 17'd0;
...
assign sram_qkm_addr_o = in_attn ? ca_qkm_addr : in_mlp ? mlp_qkm_addr : 17'd0;
...
always @(*) begin
    ...
    case (state)
        ...
        S_LOAD_X:    if (load_wp == TOK_FLAT[16:0])
                         next_state = S_NORM1;
        ...
        S_RES1:      if (res_wp == TOK_FLAT[16:0])
                         next_state = S_NORM2;
        ...
        S_RES2:      if (res_wp == TOK_FLAT[16:0])
                         next_state = S_DONE;
...
// =========================================================================
always @(*) begin
    if (state == S_LOAD_X && load_rp < TOK_FLAT[16:0]) begin
        loadx_rd_en   = 1'b1;
    ...
    if (reset) begin
        ...
        case (state)
            ...
            S_LOAD_X: begin
                ...
                if (load_rp < TOK_FLAT[16:0])
                    load_rp <= load_rp + 17'd1;
                if (load_started && load_wp < TOK_FLAT[16:0])
                    load_wp <= load_wp + 17'd1;
                load_started <= 1'b1;
            end
            ...
            S_NORM1: begin
                if (ln_y_valid)
                    ln_cap_flat <= ln_cap_flat + 17'd1;

                if (ln_start)
                    ln_cap_flat <= {tok_cnt, 8'b0};
