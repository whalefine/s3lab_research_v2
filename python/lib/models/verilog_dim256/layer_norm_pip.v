...
// layer_norm_pip.v
//
// Q8.8 LayerNorm (FEAT_DIM=256 default (dim256)), bit-accurate vs
// run_backbone_numpy_dim256_q88.py layer_norm() and verilog_backbone2/layer_norm.v.
//
...
input                       reset;
input                       start;
input  [16:0]               token_base_flat;
output reg                  x_rd_en;
output reg [16:0]           x_rd_flat;
        ...
        case (state)
            ...
            S_LOAD: begin
                if (!x_rd_pend && !x_rd_wait) begin
                    x_rd_en   <= 1'b1;
                    x_rd_flat <= token_base_flat + {{(17-FEAT_AW){1'b0}}, addr};
                    x_rd_pend <= 1'b1;
