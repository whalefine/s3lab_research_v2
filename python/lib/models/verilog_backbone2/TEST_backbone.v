...
// Compile: verilog_backbone2/*.v + memory/ ROM macros. Run simv where TXT_File/ exists.
// =============================================================================
...
initial begin
    ...
    $display("[TB] Loaded golden + template(%0d) + search(%0d)", TEMPL_TOT, SRCH_TOT);
    ...
    #(CYCLE * 80_000_000);
    $display("[TB] TIMEOUT: backbone_top did not finish within 80M cycles");
    $toggle_stop();
    $toggle_report("backbone_only_rtl.saif", 1.0e-9, "u_DUT");
    $finish;
end
