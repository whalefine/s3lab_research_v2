...
// ---------------------------------------------------------------------------
// Block weight ROM instantiations (CLK = ~clk)
// ---------------------------------------------------------------------------
...
// tok1 read addr hold (latch on x_rd_en, stable through pend/wait)
always @(posedge clk) begin
    if (reset || state != S_BACKBONE_NORM)
        bn_rd_addr_hold <= 14'd0;
    else if (bn_x_rd_en)
        bn_rd_addr_hold <= bn_x_rd_flat;
end

// tok1 registered write hold (negedge-safe for CLK~clk SRAM)
always @(posedge clk) begin
    if (reset || state != S_BACKBONE_NORM)
        bn_wr_en <= 1'b0;
