...
reg        res_rd_hold;
reg [13:0] res_rd_addr_hold;
reg        res_wr_hold;
reg [13:0] res_wr_addr_hold;
reg [15:0] res_wr_din_hold;

reg [8:0]  tok_cnt;
reg        ln_start;
reg        ln_start_r;
reg [13:0] ln_cap_flat;
reg [13:0] ln_rd_addr_hold;
...
always @(*) begin
    ...
    case (state)
        ...
        S_RES1: begin
            if (res_rd_hold) begin
                sram_tok1_ceb_o  = 1'b0;
                sram_tok1_web_o  = 1'b1;
                sram_tok1_addr_o = res_rd_addr_hold;
            end
        end
        ...
        S_RES2: begin
            if (res_wr_hold) begin
                sram_tok1_ceb_o  = 1'b0;
                sram_tok1_web_o  = 1'b0;
                sram_tok1_addr_o = res_wr_addr_hold;
                sram_tok1_din_o  = res_wr_din_hold;
            end
        end
    ...
    case (state)
        ...
        S_NORM2: begin
            if (ln_sram_rd) begin
                sram_tok2_ceb_o  = 1'b0;
                sram_tok2_web_o  = 1'b1;
                sram_tok2_addr_o = ln_rd_addr_hold;
            end
        end
        ...
        S_RES1: begin
            if (res_wr_hold) begin
                sram_tok2_ceb_o  = 1'b0;
                sram_tok2_web_o  = 1'b0;
                sram_tok2_addr_o = res_wr_addr_hold;
                sram_tok2_din_o  = res_wr_din_hold;
            end else if (res_rd_hold) begin
                sram_tok2_ceb_o  = 1'b0;
                sram_tok2_web_o  = 1'b1;
                sram_tok2_addr_o = res_rd_addr_hold;
            end
        end
    ...
    case (state)
        ...
        S_RES2: begin
            if (res_rd_hold) begin
                sram_q_ceb_o  = 1'b0;
                sram_q_web_o  = 1'b1;
                sram_q_addr_o = res_rd_addr_hold;
            end
        end
...
// =========================================================================
// S_LOAD_X registered SRAM hold (negedge-safe for real SRAM)
// =========================================================================
// tok1 read hold
always @(posedge clk) begin
    if (reset || state == S_IDLE)
        loadx_rd_en <= 1'b0;
    else if (state == S_LOAD_X && load_rp < TOK_FLAT[13:0]) begin
        loadx_rd_en   <= 1'b1;
...
// layer_norm tok2 read addr hold (latch on x_rd_en, stable through pend/wait)
always @(posedge clk) begin
    if (reset || state == S_IDLE)
        ln_rd_addr_hold <= 14'd0;
    else if ((state == S_NORM1 || state == S_NORM2) && ln_x_rd_en)
        ln_rd_addr_hold <= ln_x_rd_flat;
end

// =========================================================================
// Residual SRAM registered holds (negedge-safe for real SRAM)
// =========================================================================
// read hold (sub=0 asserts tok1/tok2 or tok2/q reads)
always @(posedge clk) begin
    if (reset || state == S_IDLE)
        res_rd_hold <= 1'b0;
    else if ((state == S_RES1 || state == S_RES2) &&
             res_sub == 2'd0 && res_rp < TOK_FLAT[13:0]) begin
        res_rd_hold      <= 1'b1;
        res_rd_addr_hold <= res_rp;
    end else
        res_rd_hold <= 1'b0;
end

// write hold (sub=3 asserts tok2 write in RES1, tok1 write in RES2)
always @(posedge clk) begin
    if (reset || state == S_IDLE)
        res_wr_hold <= 1'b0;
    else if ((state == S_RES1 || state == S_RES2) && res_sub == 2'd3) begin
        res_wr_hold      <= 1'b1;
        res_wr_addr_hold <= res_wp;
        res_wr_din_hold  <= res_y;
    end else
        res_wr_hold <= 1'b0;
end
        ...
        case (state)
            ...
            S_RES1: begin
                case (res_sub)
                    ...
                    // sub=2: detect res_v_o, hold for negedge write
                    2'd2: begin
                        if (res_v_o && res_wp < TOK_FLAT[13:0])
                            res_sub <= 2'd3;
                    end
                    // sub=3: write stable through negedge, advance ptrs
                    2'd3: begin
                        res_wp <= res_wp + 14'd1;
