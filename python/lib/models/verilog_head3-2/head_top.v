...
//
// Memory clocking: all SRAM macros in sglatrack_top use CLK(clk) (posedge,
// registered-data: addr@T -> Q@T+1). head_top drives A/CEB/WEB/D *combinationally*
// (no _n->reg stage); this presents the read addr one cycle earlier than the old
// CLK(~clk)+registered-addr path, keeping the conv/tail MAC-visible read latency at
// 1 cycle (issue@T -> x_i@T+1), so no MAC FSM retime is needed.
// =============================================================================
...
// SRAM port mux (one if / else if chain per macro).
// CLK(clk) registered-data macros: drive A/CEB/WEB/D *combinationally* (no _n->reg
// stage). Issue addr @T (comb) -> macro latches Q @posedge T+1. This matches the old
// CLK(~clk)+registered-addr latency (issue @T -> addr-on-pin @T+1 -> Q @negedge T+1),
// so conv/tail MAC pipelines see x_i with the same 1-cycle latency (no retime needed).
always @(*) begin
    tok1_ceb  = 1'b1;
    tok1_web  = 1'b1;
...
always @(*) begin
    ...
    if (c2_y_valid) begin
        sh2_ceb  = 1'b0;
        sh2_web  = 1'b0;
        sh2_addr = sh2_wr_addr;
        sh2_din  = c2_y_data;
    end else if (sh2_tail_rd_req) begin
        sh2_ceb  = 1'b0;
        sh2_web  = 1'b1;
        sh2_addr = sh2_rd_addr;
    end
end
...
always @(*) begin
    ...
    if (c1_y_valid && !sh1_wr_bank) begin
        sh1_lo_ceb  = 1'b0;
        sh1_lo_web  = 1'b0;
        sh1_lo_addr = sh1_wr_laddr;
        sh1_lo_din  = c1_y_data;
    end else if (c1_y_valid && sh1_wr_bank) begin
        sh1_hi_ceb  = 1'b0;
        sh1_hi_web  = 1'b0;
        sh1_hi_addr = sh1_wr_laddr;
        sh1_hi_din  = c1_y_data;
    end else if (sh1_c2_rd_req && !sh1_rd_bank) begin
        sh1_lo_ceb  = 1'b0;
        sh1_lo_web  = 1'b1;
        sh1_lo_addr = sh1_rd_laddr;
    end else if (sh1_c2_rd_req && sh1_rd_bank) begin
        sh1_hi_ceb  = 1'b0;
        sh1_hi_web  = 1'b1;
        sh1_hi_addr = sh1_rd_laddr;
    end
end
