...
//
// ROM read contract (ROM macro CLK(clk), posedge registered-data: addr@T -> Q@T+1) :
//   phase0 : present prefetch addr COMBINATIONALLY (w_addr_active/b_addr_active), one
//            cycle earlier than the registered w_addr_r/b_addr_r; macro registers Q at
//            the phase0->phase1 posedge.
//   phase1 : w_i / b_i valid (Q held); capture wgt_buf / bias_r. w_addr_r/b_addr_r hold
//            the same value so bank-select stays stable. 2-phase WPRE cycle count is
//            unchanged vs the old CLK(~clk) flow.
// wgt_buf: internal to conv. Activations read via parent SRAM (head_top, CLK(clk)).
//
...
// CLK(clk) ROM: present the prefetch address combinationally during phase0 (one cycle
// earlier than the registered w_addr_r/b_addr_r), so the posedge macro registers Q by
// the phase1 capture edge. w_addr_r/b_addr_r remain registered and hold the SAME value
// in phase1, so bank-select (rom_c*_use_w*) is stable across phase0->phase1.
wire             wpre_w_p0        = (CS == S_WPRE) && !wpre_done && (wpre_phase == 1'b0) ;
wire [W_AW-1:0]  wpre_w_addr_comb = (oc_base_r + wpre_lane) * FEAT_PER_OC + wpre_feat ;
wire [W_AW-1:0]  w_addr_active    = wpre_w_p0 ? wpre_w_addr_comb : w_addr_r ;
wire             bpre_b_p0        = (CS == S_WPRE) && wpre_done && !bpre_done && (bpre_phase == 1'b0) ;
wire [B_AW-1:0]  bpre_b_addr_comb = oc_base_r + bpre_lane ;
wire [B_AW-1:0]  b_addr_active    = bpre_b_p0 ? bpre_b_addr_comb : b_addr_r ;

assign rom_c1_use_w2 = w_addr_active[14] ;
assign rom_c2_use_w3 = w_addr_active[15] ;
assign rom_c2_use_w2 = w_addr_active[14] & ~w_addr_active[15] ;
assign rom_w_a       = w_addr_active[13:0] ;
assign rom_ceb_b     = !busy_r ;

generate
    if (ROM_PROFILE == 1) begin : gen_rom_c1
        assign rom_c12b_a = {1'b0, b_addr_active[6:0]} ;
        ...
        assign rom_c12b_a = 8'd96 + {2'b0, b_addr_active[5:0]} ;
