...
from lib.module import Conv2d, Linear, ReLU
...
class Corner_Predictor(nn.Module):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        ...
        self.conv5_tl = Conv2d(channel // 8, 1, kernel_size=1)

        self.conv1_br = conv(inplanes, channel, freeze_bn=freeze_bn)
        self.conv2_br = conv(channel, channel // 2, freeze_bn=freeze_bn)
        self.conv3_br = conv(channel // 2, channel // 4, freeze_bn=freeze_bn)
        self.conv4_br = conv(channel // 4, channel // 8, freeze_bn=freeze_bn)
        self.conv5_br = Conv2d(channel // 8, 1, kernel_size=1)
...
class CenterPredictor(nn.Module, ):
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        ...
        self.conv5_ctr = Conv2d(channel // 8, 1, kernel_size=1)
        ...
        self.conv5_offset = Conv2d(channel // 8, 2, kernel_size=1)
        ...
        self.conv5_size = Conv2d(channel // 8, 2, kernel_size=1)
...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, BN=False):
        ...
        if BN:
            self.layers = nn.ModuleList(nn.Sequential(Linear(n, k), nn.BatchNorm1d(k))
                                        for n, k in zip([input_dim] + h, h + [output_dim]))
        else:
            self.layers = nn.ModuleList(Linear(n, k)
                                        for n, k in zip([input_dim] + h, h + [output_dim]))
...
def build_box_head(cfg, hidden_dim):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        if cfg.MODEL.HEAD.TYPE == "CORNER":
            ...
            raise ValueError()
        return corner_head
    elif cfg.MODEL.HEAD.TYPE in ("CENTER", "CENTER_HAND"):
